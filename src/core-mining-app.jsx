...
import {
  Home, Store, Package, ArrowUpCircle, User, Zap, Battery, Database,
  Gift, Trophy, Users, Target, ChevronLeft, Bell, Settings, Sparkles,
  Gauge, ShieldCheck, Flame, Coins, Check, X, Cpu, HardDrive, Server,
  Boxes, Search, Wrench, Star, Plus
} from "lucide-react";
...
const RIG_CATALOG = [
  { key: "starter", name: "Starter Rig", rarity: "common", basePower: 4.2, baseCost: 800, desc: "Entry-level rig for new miners. Low power draw, easy to maintain." },
  { key: "pro", name: "Pro Rig", rarity: "rare", basePower: 12.5, baseCost: 3200, desc: "Balanced multi-GPU setup with solid efficiency for steady daily income." },
  { key: "hyper", name: "Hyper Rig", rarity: "epic", basePower: 34, baseCost: 9800, desc: "High-density rack with reinforced cooling for sustained heavy loads." },
  { key: "quantum", name: "Quantum Rig", rarity: "legendary", basePower: 82, baseCost: 26000, desc: "Flagship immersion-cooled unit — the highest hash power in the fleet." },
];
...
// is the in-game CORE cost. `tflops`/`vram`/`msrp` are shown as flavor text
// so the numbers are grounded in the real card, not made up.
const COMPONENT_CATALOG = [
  { key: "rtx5060", name: "RTX 5060", rarity: "common", tflops: 19.2, vram: "8GB GDDR7", msrp: 299, boostPct: 10, power: 8, price: 1200 },
  { key: "rtx5080", name: "RTX 5080", rarity: "rare", tflops: 56.3, vram: "16GB GDDR7", msrp: 999, boostPct: 28, power: 23, price: 4000 },
  { key: "rtx4090", name: "RTX 4090", rarity: "epic", tflops: 82.6, vram: "24GB GDDR6X", msrp: 1599, boostPct: 41, power: 33, price: 6400 },
  { key: "rtx5090", name: "RTX 5090", rarity: "legendary", tflops: 104.8, vram: "32GB GDDR7", msrp: 1999, boostPct: 52, power: 42, price: 8000 },
];
...
// Energy refills, sold under the Market "Packs" tab. Energy drains 0.03%/sec
// while mining and gates income at 0 — buying a pack is the only way back up.
const ENERGY_PACK_CATALOG = [
  { key: "quickcharge", name: "Quick Charge", rarity: "common", amount: 20, price: 300 },
  { key: "powercell", name: "Power Cell", rarity: "rare", amount: 50, price: 650 },
  { key: "fullcharge", name: "Full Charge", rarity: "epic", amount: 100, price: 1100 },
];
...
const DAILY_REWARDS = [
  ...
  { day: 5, amount: 260 },
  { day: 6, amount: 380 },
  { day: 7, amount: 750, bonus: "Quick Charge Pack" },
];
...
const ACHIEVEMENTS = [
  { key: "first_rig", label: "First Rig", desc: "Own at least 1 mining rig.", metric: "rigCount", target: 1, reward: 100, xp: 50, format: "count", unit: "rig" },
  { key: "component_tech", label: "Component Tech", desc: "Install at least 1 component on a rig.", metric: "installedCount", target: 1, reward: 150, xp: 60, format: "count", unit: "installed" },
  { key: "field_repair", label: "Field Repair", desc: "Repair a damaged rig at least once.", metric: "repairsCount", target: 1, reward: 100, xp: 40, format: "count", unit: "repair" },
  { key: "streak_master", label: "Streak Master", desc: "Reach a 7-day daily check-in streak.", metric: "dailyStreak", target: 7, reward: 600, xp: 150, format: "count", unit: "day streak" },
  { key: "fully_loaded", label: "Fully Loaded", desc: "Fill every component slot on one rig.", metric: "maxFillRatio", target: 1, reward: 800, xp: 200, format: "percent" },
  { key: "power_overwhelming", label: "Power Overwhelming", desc: "Reach 350 TH/s total mining power.", metric: "miningPower", target: 350, reward: 1000, xp: 250, format: "ths" },
  { key: "rig_collector", label: "Rig Collector", desc: "Own the maximum fleet of 5 rigs.", metric: "rigCount", target: 5, reward: 1500, xp: 300, format: "count", unit: "rigs" },
  { key: "six_figures", label: "Six-Figure Miner", desc: "Earn 100,000 CORE in lifetime income.", metric: "totalEarned", target: 100000, reward: 2000, xp: 400, format: "core" },
];

// Daily tasks. Progress resets to 0 (and claims unlock again) at midnight.
const MISSIONS = [
  { key: "claim3", label: "Claim Reward 3×", desc: "Claim your mining reward 3 times today.", metric: "claims", target: 3, reward: 80, xp: 30 },
  { key: "buy1", label: "Make a Purchase", desc: "Buy anything in the Market today.", metric: "purchases", target: 1, reward: 60, xp: 20 },
  { key: "maintain1", label: "Rig Maintenance", desc: "Upgrade or repair a rig today.", metric: "upgrades", target: 1, reward: 100, xp: 40 },
];
...
function GlowCard({ children, className = "", accent = C.cyan, style = {}, brackets = false }) {
  return (
    <div
      className={`relative rounded-2xl border backdrop-blur-sm ${className}`}
      style={{
  ...
  return (
    ...
    >
      {customSrc ? (
        ...
        <Icon size={Math.round(size * 0.5)} color={rar.color} strokeWidth={2} />
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      {customSrc ? (
        ...
        <Cpu size={Math.round(size * 0.5)} color={rar.color} strokeWidth={2} />
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      {customSrc ? (
        ...
        <Zap size={Math.round(size * 0.5)} color={rar.color} strokeWidth={2} />
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      {customSrc ? (
        ...
        <Battery size={Math.round(size * 0.5)} color={rar.color} strokeWidth={2} />
      )}
    </div>
  );
}
...
export default function CoreMiningApp() {
  const { tg, user, isTelegram, haptic, hapticNotify } = useTelegram();
  const [tab, setTab] = useState("home");
  const [balance, setBalance] = useState(24.85);
  const [pending, setPending] = useState(0);
  const [energy, setEnergy] = useState(92);
  const [storage, setStorage] = useState(48);
  const storageCap = 5; // CORE cap before claim needed
  ...
  const [owned, setOwned] = useState([
    { id: "seed-quantum-1", key: "quantum", name: "Quantum Rig", rarity: "legendary", level: 5, basePower: 82, baseCost: 26000, durability: 95, slots: [ "rtx5090", "rtx4090", null, null, null, null, null, null ] },
    { id: "seed-hyper-1", key: "hyper", name: "Hyper Rig", rarity: "epic", level: 4, basePower: 34, baseCost: 9800, durability: 88, slots: Array(SLOT_CAPACITY.epic).fill(null) },
    { id: "seed-pro-1", key: "pro", name: "Pro Rig", rarity: "rare", level: 3, basePower: 12.5, baseCost: 3200, durability: 100, slots: Array(SLOT_CAPACITY.rare).fill(null) },
  ]);
  const [componentInventory, setComponentInventory] = useState({}); // { [componentKey]: unassignedCount }
  ...
  const installedComponents = owned.flatMap((r) => (r.slots || []).filter(Boolean));
  const componentPower = installedComponents.reduce((sum, key) => {
    const c = COMPONENT_CATALOG.find((x) => x.key === key);
    return sum + (c ? c.power : 0);
  }, 0);
  const componentIncome = installedComponents.reduce((sum, key) => {
    const c = COMPONENT_CATALOG.find((x) => x.key === key);
    return sum + (c ? c.power * (c.boostPct / 100) : 0);
  }, 0);
  ...
  const BASE_INCOME_RATE = 0.15; // CORE earned per TH/s per hour, rigs alone (no boost baked in)

  const miningPower = owned.reduce((sum, r) => sum + rigEffectivePower(r), 0) + componentPower;
  const incomePerHour =
    (owned.reduce((sum, r) => sum + rigEffectivePower(r) * BASE_INCOME_RATE, 0) + componentIncome) *
    boosterMultiplier;
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setNowTick(Date.now());
      setActiveBooster((b) => (b && b.expiresAt <= Date.now() ? null : b));
      if (energy <= 0) return;
      setPending((p) => {
        const next = p + incomePerHour / 3600;
        return Math.min(next, storageCap);
      });
      setStorage((s) => Math.min(100, s + (incomePerHour / 3600 / storageCap) * 100 * 0.02));
      setEnergy((e) => Math.max(0, e - 0.03));
      setOwned((os) => os.map((r) => ({ ...r, durability: Math.max(0, (r.durability ?? 100) - 0.015) })));
    }, 1000);
    return () => clearInterval(id);
  }, [incomePerHour, energy]);
  ...
  const buyRig = (rig) => {
    ...
    setOwned((o) => [...o, { ...rig, id, level: 1, durability: 100, slots: Array(capacity).fill(null) }]);
  ...
  const buyComponent = (comp) => {
    ...
    setComponentInventory((inv) => ({ ...inv, [comp.key]: (inv[comp.key] || 0) + 1 }));
    setMissionProgress((mp) => ({ ...mp, purchases: mp.purchases + 1 }));
    notify(`${comp.name} added to inventory`);
  };

  const installComponent = (rigId, slotIndex, key) => {
    haptic("light");
    if ((componentInventory[key] || 0) <= 0) {
      notify("None in stock — buy one in Market", "error");
      return;
    }
    setComponentInventory((inv) => ({ ...inv, [key]: inv[key] - 1 }));
    setOwned((os) =>
      os.map((r) => (r.id === rigId ? { ...r, slots: r.slots.map((s, i) => (i === slotIndex ? key : s)) } : r))
    );
  ...
  const uninstallComponent = (rigId, slotIndex) => {
    ...
    const key = rig?.slots?.[slotIndex];
    if (!key) return;
    setOwned((os) =>
      os.map((r) => (r.id === rigId ? { ...r, slots: r.slots.map((s, i) => (i === slotIndex ? null : s)) } : r))
    );
    setComponentInventory((inv) => ({ ...inv, [key]: (inv[key] || 0) + 1 }));
  ...
  const buyEnergyPack = (pack) => {
    ...
    setEnergy((e) => Math.min(100, e + pack.amount));
    setMissionProgress((mp) => ({ ...mp, purchases: mp.purchases + 1 }));
    notify(`+${pack.amount} energy from ${pack.name}`);
  };

  const upgradeRig = (rig) => {
    haptic("light");
    const cost = Math.round(rig.baseCost * 0.7 * rig.level);
    if (balance < cost) {
      notify("Not enough CORE", "error");
      return;
    }
  ...
  const repairRig = (rig) => {
    ...
    const cost = Math.max(50, Math.round(rig.baseCost * 0.15 * (missing / 100)));
    if (balance < cost) {
      notify("Not enough CORE", "error");
      return;
    }
  ...
  const claimDaily = () => {
    ...
    if (reward.bonus) {
      // "Quick Charge Pack" day-7 bonus — top up energy directly.
  ...
  // Native Telegram MainButton — mirrors the in-app Claim button on Home.
  useEffect(() => {
    if (!tg) return;
    if (tab === "home") {
      tg.MainButton.setParams({
        text: pending > 0 ? `CLAIM ${fmt(pending, 4)} CORE` : "MINING…",
        color: "#00E5FF",
        text_color: "#04070E",
        is_active: pending > 0,
        is_visible: true,
      });
      const handler = () => claimReward();
  ...
  return (
    ...
    >
      ...
      >
        <NavItem navKey="home" icon={Home} label="Home" active={tab === "home"} onClick={() => { haptic("light"); setTab("home"); }} />
        <NavItem navKey="market" icon={Store} label="Market" active={tab === "market"} onClick={() => { haptic("light"); setTab("market"); }} />
        <NavItem navKey="inventory" icon={Package} label="Inventory" active={tab === "inventory"} onClick={() => { haptic("light"); setTab("inventory"); }} />
        <NavItem navKey="upgrade" icon={ArrowUpCircle} label="Upgrade" active={tab === "upgrade"} onClick={() => { haptic("light"); setTab("upgrade"); }} />
        <NavItem navKey="profile" icon={User} label="Profile" active={tab === "profile"} onClick={() => { haptic("light"); setTab("profile"); }} />
      </div>
    </div>
  );
}
...
// HOME
// ---------------------------------------------------------------------------
function HomeTab({ balance, pending, energy, storage, miningPower, incomePerHour, onClaim, activeBooster, nowTick, owned, featuredRigId }) {
    ...
    <div>
      ...
      <div className="px-4 mt-3">
        <GlowCard accent={C.orange} className="p-4">
          <StatBar label="ENERGY" value={energy} color={C.orange} />
...
// INVENTORY
// ---------------------------------------------------------------------------
function InventoryTab({ owned, onSelect, componentInventory = {}, featuredRigId, onSetFeatured }) {
  const stockEntries = Object.entries(componentInventory).filter(([, count]) => count > 0);
  return (
    <div>
      <TopBar title="Inventory" right={<Search size={17} />} />
...
function MarketTab({ balance, filter, setFilter, onBuy, ownedRigCount, componentInventory, onBuyComponent, activeBooster, onBuyBooster, energy, onBuyEnergyPack }) {
  ...
  return (
    <div>
      <TopBar title="Market" right={<Search size={17} />} />
        ...
        {filter === "Rigs" ? (
          ...
          COMPONENT_CATALOG.slice()
            ...
            .map((comp) => {
              ...
              const owned = componentInventory[comp.key] || 0;
            ...
            return (
              <GlowCard key={pack.key} accent={rar.color} className="p-3 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px]" style={{ color: C.orange }}>+{pack.amount} energy</p>
                  <p className="text-white text-xs font-bold tabular-nums mt-0.5">
...
// UPGRADE
// ---------------------------------------------------------------------------
function UpgradeTab({ owned, selected, onSelect, balance, onUpgrade, onRepair, componentInventory = {}, onInstall, onUninstall }) {
  ...
  const cost = Math.round(rig.baseCost * 0.7 * level);
  ...
  const usage = Math.min(95, 40 + level * 5);
  const durColor = dur <= 0 ? "#FF5252" : dur < 30 ? C.orange : C.purple;

  const repairCost = Math.max(50, Math.round(rig.baseCost * 0.15 * ((100 - dur) / 100)));
  ...
  return (
    <div>
      <TopBar title="Upgrade" right={<Search size={17} />} />
      ...
      <div className="px-4 mt-3">
        <GlowCard accent={rar.color} brackets className="p-5">
          ...
          <StatBar label="Energy Usage" value={usage} color={C.orange} />
          <StatBar label="Durability" value={dur} color={durColor} />

          {/* Component slots */}
          <div className="mt-1 mb-4">
            <p className="text-[11px] text-slate-400 tracking-wide mb-2">
              COMPONENT SLOTS ({filledSlots}/{slots.length})
            </p>
            <div className="grid grid-cols-4 gap-2">
              {slots.map((slotKey, i) => {
                if (slotKey) {
                  const c = COMPONENT_CATALOG.find((x) => x.key === slotKey);
                  return (
                    <button
                      key={i}
                      ...
                      title="Tap to remove"
                    >
                      <ComponentIcon compKey={slotKey} rarity={c?.rarity} size={48} />
                      <span
                    ...
                    {stockEntries.map(([key, count]) => {
                      ...
                      return (
                        <button
                          key={key}
                          onClick={() => {
                            onInstall(rig.id, pickerSlot, key);
                            setPickerSlot(null);
                          }}
                        ...
                        >
                          ...
                          <span className="text-[10px] font-bold" style={{ color: crar.color }}>x{count}</span>
                        </button>
                      );
                    })}
                  </div>
...
// PROFILE
// ---------------------------------------------------------------------------
function ProfileTab({ level, xp, xpToNext, totalEarned, miningPower, notify, user, isTelegram, onOpenDaily, dailyClaimAvailable }) {
  ...
  return (
    <div>
      <TopBar title="Profile" right={<Settings size={18} />} />
      ...
      <div className="px-4 grid grid-cols-2 gap-3 mt-3">
        {tiles.map((t) => {
          ...
          return (
            <button
              key={t.label}
              onClick={() => (isDaily ? onOpenDaily() : notify(`${t.label} opened`))}
            >
